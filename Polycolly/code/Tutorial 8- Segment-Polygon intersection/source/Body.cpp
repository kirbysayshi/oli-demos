...
bool CBody::Collide(CBody& xBody, float dt)
{
						  ...
						  N, t))
	{
		...
		{
			if (!m_fptrHandlePlayerContact(N, t, &xBody))
				return true;
		}

		if (xBody.m_fptrHandlePlayerContact)
		{
			if (!xBody.m_fptrHandlePlayerContact(-N, t, this))
				return true;
		}
...
void CBody::SetCollisionCallback(bool (*fptrHandlePlayerContact)(const Vector& N, float& t, CBody* pxOtherBody))
{
