...
void GameInit(void)
{
	...
	for(int i = 0; i < 2; i ++)
	{
		...
		m_fAngle[i] = 0.0f;
		m_fAngularVelocity[i] = (dbg_rotating)? (float) (i) / 2.0f + 0.1f : 0.0f;
...
void GameUpdate(float dt)
{
	...
	if (dbg_collided)
	{
		PolyColl::FindContacts(	m_pxVertices[0], m_iNumVertices[0], m_xPosition[0], m_xDisplacement[0], m_xOrientation[0], 
								m_pxVertices[1], m_iNumVertices[1], m_xPosition[1], m_xDisplacement[1], m_xOrientation[1], 
								Ncoll, tcoll, 
								CA, CB, Cnum);
	}
...
void GameRender(void)
{
	...
	if (dbg_collided)
	{
		...
		{
			RenderSegment(CA[i], CB[i], 0xFFFF0000, 0xF0F0, 4.0f, 1.0f);

			// render the collision stuff
			RenderArrow(CB[i], Ncoll, length, 0xFFFFFFFF);
		}
	...
	PrintString(1, y++, false, 0xFFFFFFFF, "Press 'P' to pause-unpause");
}
