...
bool PolyColl::ClipSegment(const Vector* A, int Anum, const Vector& xOffset, const Vector& xStart, const Vector& xEnd, float& tnear, float& tfar, Vector& Nnear, Vector& Nfar)
{
	if (!A) return false;

	Vector xPos = xStart - xOffset;
