...
{	
	...
	u_int uLine = (IsUnmovable())? 0xFF8080FF : 0xFF80FF80;

	PolyColl::Render(m_xPosition, uFill, uLine, m_axVertices, m_iNumVertices);
}
...
bool CBody::Collide(CBody& xBody)
{
	...
	if (PolyColl::Collide(m_axVertices, m_iNumVertices, 
						  xBody.m_axVertices, xBody.m_iNumVertices, 
						  xRelPos, xRelDis, 
						  N, t))
	{
		if (t < 0.0f)
