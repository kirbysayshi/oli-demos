...
struct TParticle
{
	...
	void SetCollided(bool bOnOff=true) { m_bCollided = bOnOff; }
...
//			printf("ERROR : Particle::Update()\n");
		...
		m_xNewPos  += (m_xNewPos - m_xCurrPos) + m_xForces * (m_fInvMass * (dt*dt));
		m_xCurrPos	= Temp;
		m_xForces   = Vector(0.0f, 0.0f);
...
struct TObstacle
{
	...
	void Render(CColour col=CColour(1, 1, 1, 1)) const
	{
		col.Render();
		glLineWidth(3);
		glBegin(GL_LINES);
...
//		m_xBox.Render();
	...
	bool Collide(TParticle& xParticle) const
	{
		...
		const Vector&	p0 = xParticle.GetCurrentPos();
		      Vector&	p1 = xParticle.GetTargetPos();
...
struct CTerrain
{
	...
	void Collide(TParticle* xParticles, int iNumParticles) const
	{
		for(int i =0; i < eNumObstacles; i ++)
		{
			...
			{
				if (m_xObstacles[i].Collide(xParticles[j]))
					xParticles[j].SetCollided();
			}
		}

/*		for(int j = 0; j < iNumParticles; j ++)
		{
			if (xParticles[j].GetTargetPos().y > 200.0f)
				printf("ERROR : Terrain::Collide()\n");
		}
/**/	
	}
...
struct CWheel: public CBody
{
	...
	: CBody(iNumParticles + 1, iNumParticles * 2)
	{
		...
		{
			TParticle xParticle(Vector(cos(a)* fRadius + xPos.x, sin(a) * fRadius + xPos.y), 1, 0.1f);
...
struct CCar: public CBody
{
	...
	Vector GetPos() const { return m_pxFrontWheel->GetAxle()->GetCurrentPos(); }
...
enum { eMaxPlayers = 4, eNumPlayers = 2 };
...
void display(void)
{
	...
	for(int i = 0; i < eNumPlayers; i++)
	{
		...
		gpxCamera[i]->MoveCamera(gpxCar[i]->GetPos(), 3.0f); //fZoom);
...
void animate()
{
	...
	for(int i = 0; i < eNumPlayers; i++)
	{
		...
		if (DebouncedKeyPressed(giKeys[i][2]))
		{
			...
			gpxCar[i] = new CCar(xPos, giKeys[i][1], giKeys[i][0]);
		}
	}
